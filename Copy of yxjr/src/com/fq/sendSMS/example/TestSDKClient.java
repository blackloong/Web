package com.fq.sendSMS.example;
...
import com.fq.sendSMS.*;
import com.fq.util.StringUtils;



public class TestSDKClient {
	public static String softwareSerialNo = "6SDK-EMY-6688-KIUPQ";// 软件序列号,请通过亿美销售人员获取
	public static String key = "guminzaixian2015";// 序列号首次激活时自己设定
	public static String password = "835669";// 密码,请通过亿美销售人员获取

     //用户自定义key值， 长度不超过15个字符字，和软件序列号注册时的关键字保持一致
  
	public static void main(String[] args) {
		testSendSMS("15216844605",null);
		/**
...
*/
	 ...
	 * addSerial, int smsPriority)
	 *  1、mobiles 手机数组长度不能超过1000
	 *   2、smsContent 最多500个汉字或1000个纯英文
	...
	public static void testSendSMS() {
		try {
			int i = SingletonClient.getClient().sendSMS(new String[] { "15821097871" }, "【小羊行】验证码为:1234 ", "111",5);// 带扩展码
			System.out.println("testSendSMS=====" + i);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	public static int testSendSMS(String mobile,String content) {
		...
		if(!StringUtils.NotEmpty(content)){
			content="注册验证码为: "+i+" ,有效期为2分钟";
		}
		try {
			int j= SingletonClient.getClient().sendSMS(new String[] { mobile }, "【股民在线】"+content, "1",5);// 带扩展码
			System.out.println("testSendSMS====="+mobile+"___" + j);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return i;
